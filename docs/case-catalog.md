...
| MASCOTTE G2 reduced-JL FGM probe | `examples/mascotte-g2-jl-fgm/case/` | `full_definition`; setup/readback-only execution scope | inspect an engineer-selected SRK plus reduced-JL diffusion-FGM definition without claiming historical reproduction | schema-valid and plan-compilable; two Wuzhen setup attempts reached Fluent but stopped before complete readback at runtime API/metadata boundaries | species discovery was fixed between jobs `43266182` and `43266815`; the remaining blocker is empty allowed-value evidence for the active PDF option; no calculation, initialization, iteration, or `SUCCESS` |
