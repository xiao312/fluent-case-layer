...
| MASCOTTE G2 reduced-JL FGM probe | `examples/mascotte-g2-jl-fgm/case/` | `full_definition`; setup/readback-only execution scope | inspect an engineer-selected SRK plus reduced-JL diffusion-FGM definition without claiming historical reproduction | schema-valid and plan-compilable; three Wuzhen setup attempts reached Fluent but stopped before complete readback at runtime API/metadata boundaries | species discovery is fixed; job `43271670` proved `beta` in generated and server-static v261 metadata but found the live PDF leaf inactive and unreadable, so no setter ran; no calculation, initialization, iteration, or `SUCCESS` |

`examples/campaign.yaml` compiles the first three together. The G2 probe remains
standalone while its runtime readback is blocked. Compilation or use of the
